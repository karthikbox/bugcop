thedata <- read.csv("/home/karthik/r/jdt2.csv" , header=T, sep=",")
set.seed(98052)
thedata1=thedata
for(i in 1:length(thedata[,12])){
  if(thedata[i,12]>0){
    thedata[i,12]=1
  }
  else thedata[i,12]=0
...
for(run in 1:50){
  ...
  withActualBugs=data.frame(actualBugs=thedata1[-idxs,12],withPred)
  withLoc=data.frame(loc=thedata1[-idxs,5],withActualBugs)
