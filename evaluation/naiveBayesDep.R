thedata <- read.csv("/home/karthik/r/Equinox-CK-OO-DEP3.csv" , header=T, sep=",")
thedata1=thedata
#library("e1071", lib.loc="/home/karthik/R/i686-pc-linux-gnu-library/2.14")
set.seed(98052)
N<-50
thedata[,5]=thedata[,5]>0 #change--bugs column t/f
thedata=thedata[,1:5] #change till bugs column
...
for ( i in 1:N){ 
  ...
  pred=predict(fit,test[-5]) #change bug column
  finaldata=data.frame(class=pred,bugs=thedata1[-idxs,5]) #change bug column
  finaldata=data.frame(finaldata,loc=thedata1[-idxs,6])#change loc coulmn
  ...
  filesFractionSamples[i]=files/length(test[,5]) #change bug column
  bugFractionSamples[i]=sum/bugTotal
  linesFractionSamples[i]=lines/linesTotal
  outcome<-table(true=thedata[-idxs,5],pred=pred) #change bug column
  #print(outcome)
  TN = outcome[1,1]
  FN = outcome[2,1]
  FP = outcome[1,2]
