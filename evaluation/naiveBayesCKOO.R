thedata <- read.csv("/home/karthik/r/Equinox-CK-OO-DEP.csv" , header=T, sep=",")
thedata1=thedata
#library("e1071", lib.loc="/home/karthik/R/i686-pc-linux-gnu-library/2.14")
set.seed(98052)
N<-50
thedata[,18]=thedata[,18]>0
...
# } 
    ...
    pred=predict(fit,test[-18])
    finaldata=data.frame(class=pred,bugs=thedata1[-idxs,18])
    finaldata=data.frame(finaldata,loc=thedata1[-idxs,9])
    ...
    filesFractionSamples[i]=files/length(test[,18])
