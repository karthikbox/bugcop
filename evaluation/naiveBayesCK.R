thedata <- read.csv("/home/karthik/r/Equinox-CK-OO-DEP1.csv" , header=T, sep=",")
thedata1=thedata
#library("e1071", lib.loc="/home/karthik/R/i686-pc-linux-gnu-library/2.14")
set.seed(98052)
N<-50
thedata[,7]=thedata[,7]>0 #change
thedata=thedata[,1:7] #change
...
for ( i in 1:N){ 
  ...
  finaldata=data.frame(class=pred,bugs=thedata1[-idxs,7]) #change
  finaldata=data.frame(finaldata,loc=thedata1[-idxs,8])#change
  ...
  filesFractionSamples[i]=files/length(test[,7]) #change
  bugFractionSamples[i]=sum/bugTotal
  linesFractionSamples[i]=lines/linesTotal
  outcome<-table(true=thedata[-idxs,7],pred=pred) #change
  #print(outcome)
  TN = outcome[1,1]
  FN = outcome[2,1]
  FP = outcome[1,2]
