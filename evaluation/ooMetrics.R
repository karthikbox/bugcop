thedata <- read.csv("/home/karthik/r/mylin2.csv" , header=T, sep=",")
set.seed(98052)
x<-as.data.frame(scale(thedata[,1:11]))
y<-thedata[,12]
...
for(run in 1:N){
  ...
  #fit<-lm(formula = y ~ fanOut + numberOfAttributes + numberOfLinesOfCode + 
   #         numberOfMethods + numberOfPrivateMethods + numberOfPublicMethods, 
    #      data = data[idxs, ])
  ...
  temp=data.frame(newdata2,loc=thedata[-idxs,5])
  newdata2=temp
  ...
  for(i in percentiles){
    ...
    for(j in 1:len){
      if(finalData[j,1]>=i){
        ...
        sumLines=sumLines+finalData[j,16]
      }
    }
  ...
  for(i in percentiles){
    ...
    for(j in 1:len){
      if(finalData1[j,1]>=i){
        ...
        sumLines=sumLines+finalData1[j,16]
      }
    }
