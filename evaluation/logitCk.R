thedata <- read.csv("/home/karthik/r/jdt1.csv" , header=T, sep=",")
set.seed(98052)
thedata1=thedata
for(i in 1:length(thedata[,7])){
  if(thedata[i,7]>0){
    thedata[i,7]=1
  }
  else thedata[i,7]=0
...
for(run in 1:50){
  ...
  withActualBugs=data.frame(actualBugs=thedata1[-idxs,7],withPred)
  withLoc=data.frame(loc=thedata1[-idxs,8],withActualBugs)
