thedata <- read.csv("/home/karthik/r/mylin3.csv" , header=T, sep=",")
set.seed(98052)
...
for(run in 1:N){
  ...
  #fit<-lm(formula = y ~ cbo + fanOut + lcom, data = data[idxs, ])
  ...
  temp=data.frame(newdata2,loc=thedata[-idxs,6])
  newdata2=temp
  ...
  for(i in percentiles){
    ...
    for(j in 1:len){
      if(finalData[j,1]>=i){
        ...
        sumLines=sumLines+finalData[j,9]
      }
    }
  ...
  for(i in percentiles){
    ...
    for(j in 1:len){
      if(finalData1[j,1]>=i){
        ...
        sumLines=sumLines+finalData1[j,9]
      }
    }
